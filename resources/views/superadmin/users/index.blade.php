<x-app-layout>
        ...
        <div class="p-4 sm:p-8">
            <div class="flex-1 overflow-y-auto mt-8 p-6 min-h-screen">
                ...
                <div
                    ...
                    <h2 class="text-3xl font-bold text-black">DATA USERS</h2>
                    ...
                    <div>
                        <form method="GET" action="{{ route('superadmin.users') }}">
                            ...
                            <select name="perPage" id="perPage" onchange="this.form.submit()"
                                class="border-b px-6 bg-white-100 text-gray-700 p-3 rounded-lg">
                ...
                <div id="tableContainer" class="rounded-b-lg overflow-x-auto">
                    @if ($users->isEmpty())
                        <div class="bg-white p-8 text-center rounded-lg shadow">
                            ...
                            <p class="text-gray-500 mt-2">
                                @if (request('search'))
                                    We couldn't find any users matching "{{ request('search') }}"
                                @else
                    ...
                    @else
                        <table class="min-w-full shadow table-auto">
                            ...
                            <tbody>
                                @foreach ($users as $index => $user)
                                    <tr class="bg-gray-50 border-b hover:bg-red-100 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('superadmin.users.edit', $user->id) }}"
                                                class="text-yellow-600 hover:text-yellow-900 ml-3 transition-colors duration-200">Edit</a>
                                            <button
                                                onclick="confirmDeleteUser('{{ route('superadmin.users.delete', $user->id) }}', '{{ $user->name }}', '{{ $user->email }}')"
                                                class="text-red-600 hover:text-red-900 ml-3 transition-colors duration-200">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                @if ($users->isNotEmpty())
                    <div id="paginationContainer" class="p-4">
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="text-sm text-gray-700">Showing {{ $users->firstItem() ?? 0 }} to
                                    {{ $users->lastItem() ?? 0 }} of {{ $users->total() ?? 0 }} entries</span>
                            </div>
                            <div>
                                {{ $users->appends(['perPage' => $perPage ?? 15, 'search' => $search])->links() }}
                            </div>
                        </div>
                    </div>
                @endif
    ...
    <script>
        ...
        // Double confirmation delete function for users
        function confirmDeleteUser(url, userName, userEmail) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                html: `Anda akan menghapus user berikut:<br><br>
                ...
                if (firstResult.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        html: `Anda yakin ingin menghapus user ini?<br><br>
                              <strong>Nama:</strong> ${userName}<br>
                              <strong>Email:</strong> ${userEmail}<br><br>
