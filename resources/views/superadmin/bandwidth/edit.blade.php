<x-app-layout>
            ...
            <div class="flex-1 overflow-y-auto mt-8 p-6 min-h-screen">
                ...
                @if (session('success'))
                    <div id="success-notification"
                        class="mb-2 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg shadow-lg transform transition-all duration-500 ease-in-out opacity-100 translate-x-0">
                ...
                <div class="p-4">
                    <!-- Edit Form -->
                    <form method="POST"
                        action="{{ route('superadmin.bandwidth.update', ['itemId' => $item->id, 'bandwidthId' => $bandwidth->id]) }}"
                        id="bandwidth-form">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="item_id" id="item_id"
                                ...
                                @foreach ($items as $itemOption)
                                    <option value="{{ $itemOption->id }}"
                                        {{ $itemOption->id == $item->id ? 'selected' : '' }}>
                                        {{ $itemOption->nama_barang }}</option>
                                @endforeach
                            </select>
                            @error('item_id')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="bw" id="bw"
                                class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                value="{{ old('bw', $bandwidth->bw) }}" placeholder="Masukkan bandwidth dalam Mbps"
                                required />
                            @error('bw')
                                <div class="text-red-500 text-sm mt-1 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="price" id="price"
                                class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                                value="{{ old('price', $bandwidth->price) }}" placeholder="Masukkan harga dalam IDR"
                                required />
                            @error('price')
                                <div class="text-red-500 text-sm mt-1 flex items-center">
                                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="mb-4 flex justify-between gap-4">
                            ...
                            <button type="submit" id="submit-button"
                                class="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-lg w-1/2 font-medium transition-all duration-200 transform hover:scale-105 focus:ring-4 focus:ring-blue-300 flex items-center justify-center">
    ...
    <div id="confirmation-modal"
        ...
        <div class="bg-white rounded-lg p-6 w-96 mx-4 transform transition-all duration-300 scale-95">
            <div class="flex items-center mb-4">
                ...
                <h3 class="text-xl font-semibold text-gray-900 ml-3">Konfirmasi Update Bandwidth</h3>
            </div>
            <p id="modal-message" class="text-gray-700 mb-6">Apakah Anda yakin ingin mengupdate bandwidth ini?</p>
    ...
    <div id="loading-overlay"
        ...
        <div class="bg-white rounded-lg p-6 flex items-center space-x-3">
            ...
            <span class="text-gray-700 font-medium">Mengupdate data...</span>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('bandwidth-form').addEventListener('submit', function(event) {
            ...
            document.getElementById('modal-message').innerHTML =
                `Apakah Anda yakin ingin mengupdate bandwidth menjadi <strong>${bandwidthValue} Mbps</strong> untuk item <strong>"${itemName}"</strong> dengan harga <strong>Rp ${parseInt(priceValue).toLocaleString('id-ID')}</strong>?`;
