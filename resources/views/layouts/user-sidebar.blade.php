...
<div x-data="sidebarHandler()" x-init="init()" class="relative h-full">
    ...
    <header class="fixed top-0 left-0 w-full z-50 bg-white shadow-md md:pl-4 h-20 flex items-center">
        <div class="flex items-center space-x-3 w-full">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center space-x-2 ml-4">
                <img src="/telkom.png" alt="NJKI TELKOM" class="h-20 w-auto">
    ...
    <aside x-show="open || !isMobile" x-transition:enter="transition ease-in-out duration-300"
        ...
        <nav class="flex-1 bg-white px-4 space-y-2 overflow-y-auto">
            <p class="text-red-700 text-center text-xl font-extrabold mt-6 mb-6">
                Hello, {{ auth()->user()->name }}</p>
            <h2 class="text-gray-900 text-xs font-extralight border-b uppercase mt-4 px-4 py-2">User Menu</h2>
            <x-nav-link href="{{ route('user.dashboard') }}" :active="request()->routeIs('user.dashboard')"
                class="w-full box-border text-lg flex items-center px-4 py-2 text-red-700 hover:bg-red-300 hover:font-semibold transition duration-150 ease-in-out rounded-lg"
                @click="isMobile && close()">
                <svg class="w-6 h-6 text-red-700 mr-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
            ...
            <x-nav-link href="{{ route('user.calculator') }}" :active="request()->routeIs('user.calculator')"
                class="w-full box-border text-lg items-center px-4 py-2 text-red-700 hover:bg-red-400 rounded-lg"
                @click="isMobile && close()">
                <svg class="w-6 h-6 mr-3 text-red-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                    width="24" height="24" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <x-nav-link href="{{ route('user.investment.archive.index') }}" :active="request()->routeIs('user.investment.archive.index')"
                class="w-full box-border text-lg items-center px-4 py-2 text-red-700 hover:bg-red-400 rounded-lg"
                @click="isMobile && close()">
            ...
            <h2 class="text-red-700 text-xs font-extralight border-b uppercase px-4 py-2 mt-4">Settings</h2>
            ...
            {{-- <h2 class="text-red-700 text-xs font-extralight border-b uppercase px-4 py-2 mt-4">Menu Master</h2>
